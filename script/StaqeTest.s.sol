...
import {ERC721Mock, IERC165} from "../test/mock/ERC721Mock.sol";
  ...
  *      anvil --block-time 10
  *      sleep 10 && forge script script/StaqeTest.s.sol --fork-url http://localhost:8545 --broadcast
  *
...
contract StaqeScript is Script {
    function run() external {
        ...
        ERC20Mock mockStakeToken = new ERC20Mock("Stake", "STK");
        mockStakeToken.mint(userAddress, 1000 ether);
        console.log("Stake ERC20:", address(mockStakeToken));

        ERC721Mock mockNFT = new ERC721Mock("NFT", "NFT");
        ERC721Mock(mockNFT).mint(userAddress, 1);
        ERC721Mock(mockNFT).mint(userAddress, 2);
        ERC721Mock(mockNFT).mint(userAddress, 3);
        console.log("Stake ERC721:", address(mockNFT));

        ERC20Mock mockRewardToken = new ERC20Mock("Reward", "RWD");
        mockRewardToken.mint(userAddress, 1000 ether);
        console.log("Reward ERC20:", address(mockRewardToken));

        ERC721Mock genesisNFT = new ERC721Mock("Genesis", "GNS");
        ERC721Mock(genesisNFT).mint(userAddress, 1);
        ERC721Mock(genesisNFT).mint(userAddress, 2);
